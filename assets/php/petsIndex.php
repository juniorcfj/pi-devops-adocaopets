...
function listarPets()
{
    ...
    $sql = "SELECT cp.foto, cp.nome, cp.especie,cp.estado, cp.cidade, cp.sexo, cp.idade, cp.porte, c.whatsapp FROM cadastropet cp  left join cadastro c ON cp.dono = c.id WHERE adotado = false ORDER BY cp.id desc limit 8";
